...
describe UsersController do
  ...
  it 'does not update the user admin status' do
    user = make_user_session(Factory.create(:user))

    User.stub!(:find).with(user.id.to_s).and_return(user)
    user.should_receive(:update_attributes).exactly(1).times

    post :update, {:is_admin => '1'}
...
#    user.should_receive(:update_attributes)
  ...
  it 'bootstraps the first admin' do
    user = make_user_session(Factory.create(:user))

    User.stub!(:find).with(user.id.to_s).and_return(user)
    user.should_receive(:save!).exactly(1).times

    post :admin_bootstrap, {:id => '0'}
    user.is_admin.should == true
